...
contract ShareDispenser is Ownable {
    ...
    function getAvailableALEQSupply() public view returns(uint256) {
        ERC20 a = ERC20(ALEQContractAddress);
        uint256 allowed = a.allowance(ALEQSupplyAddress, address(this));
        uint256 supply = a.balanceOf(ALEQSupplyAddress);
        return (allowed <= supply) ? allowed : supply;
    }
    
    function getCumulativeBuyPrice(uint256 numberToBuy, uint256 availableShares) public view returns(uint256){
        return sharePriceInXCHF.mul(numberToBuy);
    }

    function getAvailableXCHFSupply() public view returns(uint256) {
        ERC20 xchf = ERC20(XCHFContractAddress);
        uint256 allowed = xchf.allowance(XCHFSupplyAddress, address(this));
        uint256 supply = xchf.balanceOf(XCHFSupplyAddress);
        return (allowed <= supply) ? allowed : supply;
    }

    function getCumulativeBuyBackPrice() public view returns(uint256){
        return buyBackPriceInXCHF;
    }

    
    function getAvailableBuyBack() public view returns (uint256) {
    ...
    function buyShares(uint256 numberOfSharesToBuy) public returns(bool) {
        ...
        uint256 availableSupply = getAvailableALEQSupply();
    ...
    //     uint256 availableBuyBack = getAvailableBuyBack();
    //     // uint256 priceInXCHF = getCumulativeBuyPrice(numberOfSharesToBuy, availableSupply);
